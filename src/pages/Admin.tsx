    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '12px', padding: '3rem', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" style={{ width: '100%', padding: '0.75rem', backgroundColor: '#D4AF37', color: '#000', border: 'none', borderRadius: '6px', fontWeight: 700, fontSize: '1rem', cursor: 'pointer' }}>
            Enter
          </button>
        </form>
      </div>
...
function SessionDashboard() {
  ...
  const parseSessionLog = (raw: string, id: string): SessionLog => {
    ...
    const extractBullets = (section: string | undefined) => {
      if (!section) return [];
      return section.split('\n').filter(l => l.trim().match(/^[-*]\s/)).map(l => l.replace(/^[-*]\s+/, '').trim()).filter(Boolean);
    };
    return {
      id,
      title: titleMatch?.[1]?.trim() || 'Untitled Session',
      date: dateMatch?.[1]?.trim() || new Date().toLocaleDateString(),
      summary: summaryMatch?.[1]?.trim().split('\n').slice(0, 3).join(' ') || raw.slice(0, 200),
      nextSteps: extractBullets(nextStepsMatch?.[2]),
      files: extractBullets(filesMatch?.[2]),
      raw,
    };
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach(file => {
      ...
      reader.onload = (ev) => {
        ...
        const log = parseSessionLog(content, Date.now().toString());
  ...
  const handlePasteImport = () => {
    ...
    const log = parseSessionLog(pasteText, Date.now().toString());
    ...
    <div>
      ...
      {pasteMode && (
        <div style={{ ...card, cursor: 'default', marginBottom: '2rem' }}>
          <p style={{ color: '#a1a1aa', fontSize: '0.85rem', marginBottom: '0.75rem' }}>Paste session log content below (Markdown):</p>
          <textarea value={pasteText} onChange={e => setPasteText(e.target.value)} rows={8} style={{ width: '100%', backgroundColor: '#09090b', border: '1px solid #3f3f46', borderRadius: '6px', color: '#d4d4d8', padding: '0.75rem', fontSize: '0.85rem', fontFamily: 'monospace', resize: 'vertical', boxSizing: 'border-box' }} placeholder="# Session Title" />
      ...
      {selected && (
        <div style={{ ...card, cursor: 'default', borderColor: '#2563eb', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1.5rem' }}>
            <div>
              <h3 style={{ color: '#fff', margin: 0, fontSize: '1.2rem' }}>{selected.title}</h3>
              <span style={{ color: '#a1a1aa', fontSize: '0.85rem' }}>{selected.date}</span>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => navigator.clipboard.writeText(selected.raw)} style={{ padding: '0.4rem 0.8rem', backgroundColor: 'transparent', border: '1px solid #3f3f46', borderRadius: '5px', color: '#a1a1aa', cursor: 'pointer', fontSize: '0.8rem' }}>Copy Full Log</button>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <h4 style={{ color: '#D4AF37', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>Summary</h4>
            <p style={{ color: '#d4d4d8', fontSize: '0.95rem', lineHeight: 1.6 }}>{selected.summary}</p>
          </div>
          {selected.nextSteps.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              <h4 style={{ color: '#D4AF37', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.75rem' }}>Next Steps</h4>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {selected.nextSteps.map((step, i) => (
                  <li key={i} style={{ color: '#d4d4d8', fontSize: '0.9rem', padding: '0.35rem 0', borderBottom: '1px solid #27272a', display: 'flex', gap: '0.5rem' }}>
                    <span style={{ color: '#2563eb', fontWeight: 700 }}>-&gt;</span>{step}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {selected.files.length > 0 && (
            <div>
              <h4 style={{ color: '#D4AF37', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.75rem' }}>Key Files</h4>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {selected.files.map((file, i) => (
                  <li key={i} style={{ color: '#a1a1aa', fontSize: '0.85rem', fontFamily: 'monospace', padding: '0.25rem 0' }}>{file}</li>
                ))}
              </ul>
            </div>
          )}
      ...
      {sessions.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#52525b' }}>
          <p style={{ fontSize: '1rem', margin: 0 }}>No session logs yet.</p>
          <p style={{ fontSize: '0.85rem', marginTop: '0.5rem' }}>Upload a .md session log or paste one above.</p>
        </div>
      ) : (
          ...
          <div key={session.id} style={{ ...card, borderColor: selected?.id === session.id ? '#2563eb' : '#27272a' }} onClick={() => setSelected(session.id === selected?.id ? null : session)}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <h3 style={{ color: '#fff', margin: '0 0 0.35rem', fontSize: '1rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.title}</h3>
                <span style={{ color: '#52525b', fontSize: '0.8rem' }}>{session.date}</span>
                <p style={{ color: '#a1a1aa', fontSize: '0.85rem', margin: '0.5rem 0 0', lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.summary.slice(0, 120)}</p>
              </div>
              {session.nextSteps.length > 0 && (
                <span style={{ backgroundColor: '#1e3a5f', color: '#60a5fa', fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '20px', marginLeft: '1rem', whiteSpace: 'nowrap' }}>{session.nextSteps.length} next steps</span>
              )}
            </div>
          </div>
        ))
...
function RunOfShowGenerator() {
  ...
  const set = (field: keyof RunOfShow) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
  ...
  const generateROS = () => {
    const doc = [
      ...
      '**Date:** ' + (form.eventDate || '___'),
      '**Venue:** ' + (form.venue || '___'),
      '**Event Type:** ' + form.eventType,
      '**Guest Count:** ' + (form.guestCount || '___'),
      '',
      '---',
      '',
      '## TIMELINE',
      ...
      '| ' + (form.dinnerTime || '___') + ' | Dinner Service | Style: ' + form.dinnerStyle + ' |',
      ...
      '',
      '## SPECIAL REQUESTS',
      form.specialRequests || 'None listed.',
      '',
      '## NOTES',
      form.notes || 'None.',
      '',
  ...
  const downloadGenerated = () => {
    ...
    a.href = url;
    a.download = 'run-of-show-' + (form.clientName.replace(/\s+/g, '-').toLowerCase() || 'event') + '-' + (form.eventDate || 'undated') + '.md';
    a.click();
  ...
  const lS: React.CSSProperties = { color: '#a1a1aa', fontSize: '0.8rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '0.4rem' };
  ...
  return (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ color: '#fff', fontSize: '1.4rem', margin: '0 0 0.4rem' }}>Run of Show Generator</h2>
        <p style={{ color: '#a1a1aa', margin: 0, fontSize: '0.9rem' }}>Fill in the event details to generate a formatted run of show document.</p>
      </div>
        ...
        <div>
          <div style={sS}>
            ...
            <div><label style={lS}>Event Type</label>
              <select style={iS} value={form.eventType} onChange={set('eventType')}>
                <option>Wedding</option><option>Corporate Gala</option><option>Private Party</option><option>Fundraiser</option><option>Bar/Bat Mitzvah</option><option>Other</option>
              </select>
            </div>
          </div>
          <div style={sS}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1rem' }}>
              ...
              <div><label style={lS}>Cocktail Style</label>
                <select style={iS} value={form.cocktailStyle} onChange={set('cocktailStyle')}>
                  <option>Jazz/Standards</option><option>Yacht Rock</option><option>Acoustic Pop</option><option>Background DJ</option><option>Classic Rock</option>
                </select>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              ...
              <div><label style={lS}>Dinner Style</label>
                <select style={iS} value={form.dinnerStyle} onChange={set('dinnerStyle')}>
                  <option>Background Jazz</option><option>Light Pop</option><option>Acoustic</option><option>Background DJ</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div style={sS}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={lS}>Special Requests / Do Not Play</label>
              <textarea style={{ ...iS, resize: 'vertical', fontFamily: 'inherit' }} rows={3} value={form.specialRequests} onChange={set('specialRequests')} placeholder="Any special requests or DNP list..." />
            </div>
            <div>
              <label style={lS}>Internal Notes</label>
              <textarea style={{ ...iS, resize: 'vertical', fontFamily: 'inherit' }} rows={3} value={form.notes} onChange={set('notes')} placeholder="Logistics, contacts, setup notes..." />
            </div>
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <button onClick={() => setForm(emptyROS)} style={{ padding: '0.85rem 1.5rem', backgroundColor: 'transparent', border: '1px solid #3f3f46', borderRadius: '8px', color: '#a1a1aa', cursor: 'pointer' }}>Clear Form</button>
      </div>
      {generated && (
        <div style={{ backgroundColor: '#0d0d0d', border: '1px solid #27272a', borderRadius: '10px', padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => { navigator.clipboard.writeText(generated); setCopied(true); setTimeout(() => setCopied(false), 2000); }} style={{ padding: '0.5rem 1rem', backgroundColor: copied ? '#166534' : 'transparent', border: '1px solid #3f3f46', borderRadius: '5px', color: copied ? '#4ade80' : '#a1a1aa', cursor: 'pointer', fontSize: '0.85rem' }}>
                {copied ? 'Copied!' : 'Copy'}
              </button>
              <button onClick={downloadGenerated} style={{ padding: '0.5rem 1rem', backgroundColor: '#D4AF37', border: 'none', borderRadius: '5px', color: '#000', cursor: 'pointer', fontWeight: 700, fontSize: '0.85rem' }}>Download .md</button>
            </div>
          </div>
...
export default function Admin() {
  ...
  const tabBtn = (id: Tab, label: string) => (
    <button onClick={() => setTab(id)} style={{ padding: '0.6rem 1.5rem', backgroundColor: tab === id ? '#D4AF37' : 'transparent', color: tab === id ? '#000' : '#a1a1aa', border: tab === id ? 'none' : '1px solid #3f3f46', borderRadius: '6px', fontWeight: tab === id ? 700 : 400, cursor: 'pointer', fontSize: '0.9rem' }}>
      {label}
    </button>
  );
