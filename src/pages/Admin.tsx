...
function ClaudeLog() {
  ...
  const save = (next: LogEntry[]) => { setEntries(next); localStorage.setItem(LOG_KEY, JSON.stringify(next)); };
  ...
  const importEntries = () => {
    try {
      ...
      const merged = [...parsed.filter(e=>!existingIds.has(e.id)), ...entries];
      merged.sort((a,b)=>b.timestamp.localeCompare(a.timestamp));
      save(merged);
      setImportText(''); setShowImport(false); setImportError('');
    } catch { setImportError('Invalid JSON — paste the exported log from the other machine.'); }
  };
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'2rem' }}>
        <div>
          ...
          <p style={{ color:'#a1a1aa', margin:'0.4rem 0 0', fontSize:'0.9rem' }}>Track what each machine worked on. Export JSON to sync with another machine.</p>
        </div>
        <div style={{ display:'flex', gap:'0.75rem' }}>
      ...
      {showImport && (
        <div style={{ backgroundColor:'#18181b', border:'1px solid #27272a', borderRadius:'10px', padding:'1.5rem', marginBottom:'1.5rem' }}>
          <p style={{ color:'#a1a1aa', fontSize:'0.85rem', margin:'0 0 0.75rem' }}>Paste the exported JSON from another machine to merge entries:</p>
          <textarea value={importText} onChange={e=>{setImportText(e.target.value);setImportError('');}} rows={6}
            ...
            placeholder='[{"id":"...","timestamp":"...","machine":"...","summary":"..."}]' />
      ...
      {showAdd && (
        <div style={{ backgroundColor:'#18181b', border:'1px solid #D4AF37', borderRadius:'10px', padding:'1.5rem', marginBottom:'1.5rem' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.75rem', marginBottom:'0.75rem' }}>
            <div><label style={lS}>Machine / Location</label><input style={iS} value={form.machine} onChange={e=>setForm(f=>({...f,machine:e.target.value}))} placeholder="e.g. Home iMac, Work Laptop" /></div>
            <div><label style={lS}>Project / Context</label><input style={iS} value={form.context} onChange={e=>setForm(f=>({...f,context:e.target.value}))} placeholder="e.g. Harborline site, Admin build" /></div>
          </div>
          ...
          <div style={{ marginBottom:'0.75rem' }}><label style={lS}>Next Steps</label><textarea style={{ ...iS, resize:'vertical', fontFamily:'inherit' }} rows={2} value={form.nextSteps} onChange={e=>setForm(f=>({...f,nextSteps:e.target.value}))} placeholder="What to pick up next time..." /></div>
          <div style={{ marginBottom:'1rem' }}><label style={lS}>Tags (comma separated)</label><input style={iS} value={form.tags} onChange={e=>setForm(f=>({...f,tags:e.target.value}))} placeholder="website, admin, github" /></div>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'start' }}>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', marginBottom:'0.25rem' }}>
                  <span style={{ backgroundColor:'#27272a', color:'#D4AF37', fontSize:'0.75rem', padding:'0.15rem 0.6rem', borderRadius:'20px', whiteSpace:'nowrap', fontWeight:600 }}>{entry.machine}</span>
                  {entry.context && <span style={{ color:'#52525b', fontSize:'0.8rem' }}>{entry.context}</span>}
                  <span style={{ color:'#3f3f46', fontSize:'0.75rem', marginLeft:'auto' }}>{dateStr}</span>
                </div>
                <p style={{ color:'#a1a1aa', fontSize:'0.9rem', margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:isOpen?'normal':'nowrap' }}>{entry.summary}</p>
              </div>
            </div>
            {isOpen && (
              <div style={{ marginTop:'1rem', borderTop:'1px solid #27272a', paddingTop:'1rem' }}>
                {entry.nextSteps && <div style={{ marginBottom:'0.75rem' }}><p style={{ color:'#D4AF37', fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.08em', margin:'0 0 0.4rem' }}>Next Steps</p><p style={{ color:'#d4d4d8', fontSize:'0.88rem', margin:0, lineHeight:1.6, whiteSpace:'pre-wrap' }}>{entry.nextSteps}</p></div>}
                {entry.tags.length>0 && <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap', marginBottom:'0.75rem' }}>{entry.tags.map((t,i)=><span key={i} style={{ backgroundColor:'#1e3a5f', color:'#60a5fa', fontSize:'0.73rem', padding:'0.15rem 0.5rem', borderRadius:'20px' }}>{t}</span>)}</div>}
